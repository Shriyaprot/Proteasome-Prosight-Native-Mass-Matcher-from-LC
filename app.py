...
ROOT_THEORY = APP_DIR / "mouse_proteasome_reference_library.csv"
DATA_THEORY = APP_DIR / "data" / "mouse_proteasome_reference_library.csv"
...
RESULT_COLUMNS = [
    "Subunit", "Accession", "Variant", "Modification", "Common_Tissue_Form",
    "Calculated_Mass_Da", "Detected_Mass_Da", "Difference_Da", "Difference_ppm", "Category",
    "Charge_State_Range", "Charge_State_Count", "Sum_Intensity", "Relative_Intensity_pct",
    "Possible_Explanation", "Nearest_Subunit", "Nearest_Accession", "Nearest_Variant",
    "Nearest_Modification", "Nearest_Reference_Mass_Da", "Distance_to_Nearest_Da",
]

MATCHED_COLUMNS = [
    "Subunit", "Accession", "Variant", "Modification", "Common_Tissue_Form", "Calculated_Mass_Da",
    "Occurrence_Count", "Detected_Mass_Da", "Difference_Da", "Difference_ppm",
    "Charge_State_Range", "Charge_State_Count", "Sum_Intensity", "Relative_Intensity_pct",
    "Age_Group", "Organ", "Peak", "Report_File", "Data_File_Name",
]

TRACKER_COLUMNS = [
    "Cluster_Mass_Da", "Occurrence_Count", "Detected_Mass_Da", "Charge_State_Range", "Charge_State_Count",
    "Sum_Intensity", "Relative_Intensity_pct", "Age_Group", "Organ", "Peak", "Report_File", "Data_File_Name",
    "Category", "Possible_Explanation", "Nearest_Subunit", "Nearest_Accession", "Nearest_Variant",
    "Nearest_Modification", "Nearest_Reference_Mass_Da", "Distance_to_Nearest_Da",
]
...
def read_theory(uploaded_file=None) -> pd.DataFrame:
    """Read the normalized detailed mouse proteasome reference library."""
    ...
    aliases = {"Calculated Mass (Da)": "Calculated_Mass_Da", "MW": "Calculated_Mass_Da"}
    ...
    if missing:
        raise ValueError("Reference library is missing: " + ", ".join(missing))
    for c in ["Subunit","Accession","Variant","Modification","Common_Tissue_Form","Sequence"]:
        if c not in theory.columns: theory[c] = ""
        theory[c] = theory[c].fillna("").astype(str)
    theory["Calculated_Mass_Da"] = pd.to_numeric(theory["Calculated_Mass_Da"], errors="coerce")
    keep = ["Subunit","Accession","Variant","Modification","Calculated_Mass_Da","Common_Tissue_Form","Source_Row","Source_Column","Sequence"]
    keep = [c for c in keep if c in theory.columns]
...
def match_masses(df, theory, tolerance_mode, direct_tolerance, near_tolerance, shift_ppm):
    """Match against every detailed reference proteoform. Unassigned rows never masquerade as identifications."""
    if df.empty: return df.copy()
    t=theory.reset_index(drop=True); shifts=COMMON_SHIFTS.reset_index(drop=True); rows=[]
    def metric(abs_da, calc): return abs_da/calc*1_000_000.0 if tolerance_mode=="ppm" else abs_da
    for _,row in df.iterrows():
        detected=float(row["Detected_Mass_Da"]); rec=row.to_dict(); candidates=[]
        for ti,ref in t.iterrows():
            calc=float(ref["Calculated_Mass_Da"]); diff=detected-calc; score=metric(abs(diff),calc)
            common=0 if str(ref.get("Common_Tissue_Form","No")).lower()=="yes" else 1
            candidates.append((score,abs(diff),common,ti,diff))
        candidates.sort(key=lambda x:(x[0],x[1],x[2])); score,_,_,nearest_ti,diff=candidates[0]
        nearest=t.iloc[nearest_ti]; calc=float(nearest["Calculated_Mass_Da"])
        # Identification fields start blank unless a direct or putative shift assignment is made.
        rec.update({"Subunit":"","Accession":"","Variant":"","Modification":"","Common_Tissue_Form":"",
                    "Calculated_Mass_Da":np.nan,"Difference_Da":np.nan,"Difference_ppm":np.nan,
                    "Category":"Unassigned / investigate","Possible_Explanation":"",
                    "Nearest_Subunit":nearest.get("Subunit",""),"Nearest_Accession":nearest.get("Accession",""),
                    "Nearest_Variant":nearest.get("Variant",""),"Nearest_Modification":nearest.get("Modification",""),
                    "Nearest_Reference_Mass_Da":calc,"Distance_to_Nearest_Da":diff})
        if score <= direct_tolerance:
            rec.update({"Subunit":nearest.get("Subunit",""),"Accession":nearest.get("Accession",""),
                        "Variant":nearest.get("Variant",""),"Modification":nearest.get("Modification",""),
                        "Common_Tissue_Form":nearest.get("Common_Tissue_Form",""),"Calculated_Mass_Da":calc,
                        "Difference_Da":diff,"Difference_ppm":ppm_error(detected,calc),"Category":"Matched"})
            rows.append(rec); continue
        if score <= near_tolerance: rec["Category"]="Near theoretical / investigate"
        best=None
        for ti,ref in t.iterrows():
            base=float(ref["Calculated_Mass_Da"])
            for _,sh in shifts.iterrows():
                for direction in (1.0,-1.0):
                    expected=base+direction*float(sh["Shift_Da"]); resid=detected-expected; rp=ppm_error(detected,expected)
                    if best is None or abs(rp)<best[0]: best=(abs(rp),ti,sh,direction,resid,rp)
        if best and best[0] <= shift_ppm:
            _,ti,sh,direction,resid,rp=best; ref=t.iloc[ti]; base=float(ref["Calculated_Mass_Da"]); sign="+" if direction>0 else "−"
            rec.update({"Subunit":ref.get("Subunit",""),"Accession":ref.get("Accession",""),"Variant":ref.get("Variant",""),
                        "Modification":ref.get("Modification",""),"Common_Tissue_Form":ref.get("Common_Tissue_Form",""),
                        "Calculated_Mass_Da":base,"Difference_Da":detected-base,"Difference_ppm":ppm_error(detected,base),
                        "Category":"Possible PTM" if sh["Shift_Type"]=="PTM" else "Possible adduct",
                        "Possible_Explanation":f"{sign}{sh['Shift_Name']} ({sign}{float(sh['Shift_Da']):.4f} Da); residual {resid:+.4f} Da ({rp:+.2f} ppm)"})
        rows.append(rec)
...
def compile_matched_subunits(results: list[pd.DataFrame], metas: list[dict[str, str]]) -> pd.DataFrame:
    ...
    group_cols = ["Subunit", "Accession", "Variant", "Modification"]
    ...
    out = out[keep].sort_values(
        ["Subunit", "Variant", "Modification", "Organ", "Age_Group", "Peak", "Detected_Mass_Da"],
        na_position="last",
...
def to_excel_bytes(
    ...
    ref_ws = wb.create_sheet("Reference library")
    write_dataframe_sheet(ref_ws, theory)
...
with st.sidebar:
    ...
    st.caption(
        "Direct matching uses the detailed reference masses only; historical measured masses and STD values are not used. Highlighted source-workbook forms are retained as Common_Tissue_Form metadata, not as a looser matching rule. "
...
st.subheader("1. Detailed mouse proteasome reference library")
use_default = st.checkbox("Use bundled detailed mouse proteasome reference library", value=True)
theory_upload = None
if not use_default:
    theory_upload = st.file_uploader("Upload normalized reference library", type=["csv", "xlsx", "xls"], key="theory")
...
try:
    ...
    st.error(str(exc))
    st.stop()
with st.expander(f"View reference library ({len(theory)} masses)"):
...
excel = to_excel_bytes(results, metas, candidate_tracker, matched_subunits, theory)
c1, c2 = st.columns(2)
with c1:
    st.download_button(
